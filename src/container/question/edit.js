...
export default function Index({initialValues}) {
    ...
    useEffect( ()=>{
        ...
        axios.get(process.env.REACT_APP_SITE_URL+'/questions/73')
        .then(response=>{
            let question = {
    ...
    return (
        <div>
            ...
            {res && <Toast show={showA} onClose={toggleShowA} style={{position: 'fixed', right: '20px'}}>
                <Toast.Header>
                    <strong className="mr-auto">Response</strong>
                </Toast.Header>
                <Toast.Body>{'result' in res ? <p>{res.result.title} , {res.result.qType}</p> : res}</Toast.Body>
            </Toast>}
            ...
            {({
                ...
                <Form onSubmit={handleSubmit}>
                    <MultiStep showNavigation={true} steps={[
                        {
                        name: 'StepOne',
                        component: <StepOne categories={categories} values={values} errors={errors} handleChange={handleChange} />
                        },
                        {
                            name: 'StepTwo',
