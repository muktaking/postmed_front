...
const QuestionUpload = ({viewHandler}) => {
    ...
    const [formErrors, setFormErrors] = useState({category: undefined, file: undefined});
    ...
    const handleChange = (e) => {
        ...
        switch (name) {       
          case "category":
            errors.category = validator.isInt(value) || validator.isMongoId(value)
                ? undefined
    ...
    const uploadHandler = (e) => {
        ...
        if(valid){
            ...
            setRes("Please fill form corectly" + (error ? " __ " + error.toUpperCase() + " __" : ""));
        }
  ...
  return (
      <div>
        ...
        {<Toast show={res} onClose={closeRes} 
        style={{height: '100vh', position: 'fixed', right: '20px', overflowY: 'scroll', overflowX: 'hidden'}}
        >
            <Toast.Header>
                <strong className="mr-auto">Response</strong>
            </Toast.Header>
            <Toast.Body>
                <p>{ res + ' ' }{uploadedExcelQuestion.length > 0 && uploadedExcelQuestion.length }</p>
                <div>
                    {uploadedExcelQuestion.length > 0 && uploadedExcelQuestion.map((ques, ind)=><MiniManyQues key={ind} index={ind} question={ques} viewHandler={viewHandler} />)}
                </div>
            </Toast.Body>
        </Toast>}
