...
const images = ['boy','boy1', 'girl', 'girl1','hacker', 'man','woman','woman1','woman2','soccer-player','bear'];
    ...
    return (
        <div>
          ...
          <Modal show={show} onHide={handleClose}>
            ...
            <Modal.Body>
              <div>
                ...
                {
                  images.map((v,i)=><Image key={i} thumbnail={true} src={"/assets/image/avatar/" + v + '.png'} roundedCircle className={"mb-2 mr-2 " + (pickedImg === v && "bg-primary p-2")} style={{width: '64px', height: '64px', cursor: 'pointer'}} onClick={()=>{
                    setPickedImage(v);
                  }} />)
                }
            ...
            <Modal.Footer>
              ...
              <Button variant="primary" onClick={()=>{
                ...
                axios.post(process.env.REACT_APP_SITE_URL + '/users/avatar/' + pickedImg)
                .then(response=>{
          ...
          <div className={"d-flex flex-column align-items-center text-center mb-3" + (isProfile ? '' : " bottom-border")}>
            <div>
              <img src={"/assets/image/avatar/" + (user.avatar ? user.avatar : 'boy') + ".png"} alt="Avatar" className="rounded-circle" width={size} />
              {isProfile && <AiFillEdit size="1.7rem" style={{cursor: 'pointer'}} onClick={handleShow} />}
            </div>
